...
- `📲 Получить VPN` → подменю:
  - `🔗 Подключить VPN` — одна ссылка для всех устройств (subscription `/sub/<token>`, телефон+ПК) — **главный путь**
  - `💻 AmneziaWG` — отдельный конфиг, макс. скорость ПК/Wi-Fi (⚠️ не работает на мобильном интернете)
  - `📡 Мобильный` — VLESS-REALITY под оператора (main/yc)
- `🔄 Не работает` → подменю:
...
10. **Cron на Fornex (root):**
    ...
    - `0 */6 * * *` `scripts/sync_xray_users.py --all --no-shared` — safety net: пересинхронизирует per-user VLESS UUIDs из БД в Xray config.json на main+yc. Защита от drift (ручные правки конфига / неудачные sync). 4 раза/день × ~5 сек downtime = 20 сек/день суммарно.
