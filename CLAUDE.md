...
- **Xray access-лог (с 06-17)** пишется в `/var/log/xray/access.log` (НЕ journald; `log.access` + `loglevel: warning` на eu1/main) + `logrotate /etc/logrotate.d/xray` (copytruncate, rotate 3). Поэтому journald больше не пухнет от Xray (~1М строк/день убрали). `ip_usage_watcher` читает этот файл.
...
| **Timeweb main** (RU + Мегафон/Yota) | `81.200.146.32` | **Xray VLESS+REALITY на 443** (SNI=deepl.com/fp=qq с 06-28; было cloud.mail.ru — для Мегафон/Yota при БС, см. SESSION_SUMMARY_2026-05-21). Также WireGuard wg0 на UDP/51820 для 1 legacy user. SSH: `ssh -i ~/.ssh/id_ed25519_main root@81.200.146.32` (или через Fornex jump) |
...
- ~~Yandex Cloud yc / yc2~~ — **снесены 2026-06-28** (снос YC); SSH-алиасы и ключи мертвы.
...
**Покрытие (обновлено 06-28):** блок 25.06 оказался по **SNI/fp** (заезженные `microsoft`+`chrome`), НЕ по IP/транспорту — это переопределило прежний вывод «РКН по IP-репутации» (память `project_vpn_reality_revival_sni`). Сейчас несёт **eu1 REALITY** (ebay/firefox) — проверено владельцем на мобиле И Wi-Fi — + **AmneziaWG** (eu1, UDP-резерв); main (deepl/qq) — РФ-опция. **Мегафон при БС — известный гэп.** Жёсткий whitelist-БС (Аня) не пробит ничем (память `project_vpn_hard_whitelist_unbeaten`) — покрытие НЕ считать «закрытым».
...
scripts/
  ...
  health_check.py       — 21 проверка инфры (Fornex local + main через SSH; yc/yc2 убраны 06-28) → TG-алерты владельцу (cron */15)
  ...
  sync_xray_users.py    — синхронизация per-user VLESS UUIDs из БД в Xray config.json на main через SSH + restart (yc убран 06-28; eu1 — отдельно sync_eu1_vless.py). `--server main | --all`, `--dry-run`, `--no-shared`. Также idempotent гарантирует `policy.levels.0.statsUser*` для per-user телеметрии. Использует тот же SQL-фильтр что enforce_expired (grace 12h) → revoked юзер автоматически вылетает из Xray.
...
- **Subscription-URL (validated 2026-05-25):** `GET /sub/<sub_token>` → base64-список VLESS-REALITY ссылок (eu1 `ebay.com`/firefox + main `deepl.com`/qq с 06-28; было YC microsoft + main cloud.mail.ru). Happ импортирует как «subscription» — один URL, авто-выбор сервера, авто-обновление. **Happ — референс-клиент (стандартизировано 06-20);** V2Box и пр. не всегда тянут наш xhttp REALITY → в инструкциях только Happ (память `project_vpn_client_happ_reference`). Гейт `db_is_access_active` — истёк срок → пустая подписка → все устройства отваливаются (чистый enforcement для Фазы 4). Спайк: общие share-ссылки; per-user UUID — TODO. `ProxyFix(x_proto, x_host)` в app.py чтобы за nginx Flask отдавал https-ссылки.
...
- `📲 Получить VPN` → **сразу отдаёт `🔗 Подключить VPN`** (subscription `/sub/<token>`, телефон+ПК — главный путь, как в ЛК «всё на первом экране») + кнопка `🔌 Другие способы` → подменю `💻 AmneziaWG` (макс. скорость ПК/Wi-Fi, ⚠️ не для мобильного) + `🖥 Мои устройства`. **Операторский «📡 Мобильный» флоу удалён 2026-06-28** (не нужен — основная подписка + AmneziaWG-резерв покрывают)
- `🔄 Не работает` → подменю:
...
**Команды владельца:** `/proxy_rotate`, `/broadcast`. **Админ-панель** (⚙️ Администратор) — в т.ч. **🎁 Дать всем дней** (массовое +N к `expires_at` активным юзерам, `db_bulk_extend_active`; хирургично — status/plan/триал не трогает; для компенсаций простоя).
...
# VLESS+REALITY (бывш. Yandex Cloud yc — СНЕСЁН 06-28; share-url ниже мёртв, fallback на eu1)
...
10. **Cron на Fornex (root):**
    ...
    - `*/15 * * * *` `scripts/health_check.py` — 21 проверка инфры (yc/yc2 убраны 06-28, снос YC; fail2ban на main). **Fornex (14, локально):** systemd сервисы (vpn-bot/vpn-web/nginx/xray), docker (amnezia-awg2/mtproxy-faketls), AWG peer-count, peers.json/awg consistency, диск, swap, LE-cert, HTTPS endpoint, vless_config_consistency, **`vless_traffic_flow`** (с 2026-06-26, репойнт 06-28: возраст последнего реального `@kronos`-коннекта в access.log на **eu1(локально)+main**; FAIL если eu1 молчит ≥3ч при живом xray, свежий main перекрывает — детектор сетевого блока транспорта VLESS, слепое пятно «порт+сервис» не ловит; вскрыто инцидентом 06-25, см. `project_vpn_reality_revival_sni`). **main (7, через SSH):** reachable + xray + wg-quick@wg0 + fail2ban + :443/tcp + :51820/udp + диск. **Все remote-проверки одного хоста идут одним SSH-batch'ем** (маркеры `<<<BEGIN:N>>>`/`<<<END:N:rc>>>`) — отдельные ssh-коннекты иногда отбивались провайдером main на 5-6-м соединении (false-FAIL'ы 2026-05-31). При смене статуса OK↔FAIL шлёт TG-алерт владельцу (прямой HTTP API). State: `/var/lib/vpn-health/state.json`. Remote-ключи префиксованы `<host>:` (`main:xray.service`). FAIL `<host>:reachable` — gate: остальные проверки хоста скипаются. **Кросс-серверная `vless_config_consistency`** (с 2026-06-03): count(active vless_uuid_main в users) vs count(clients в Xray config на main) — FAIL если diff > 2. На когортах истечений БД и config законно расходятся в окне grace→sync (ежечасный sync сузил окно до ~1 ч).
    ...
    - `*/10 * * * *` `scripts/ip_usage_watcher.py` — **iplimit-НАБЛЮДЕНИЕ (Фаза 2 A-Stage1)**, ТОЛЬКО замер. Парсит access-log Xray **из файла** `/var/log/xray/access.log` (не journald, с 06-17) на входах (eu1 локально + main SSH; yc/yc2 убраны 06-28), копит (real client IP ↔ tid) в `ip_usage` (релей `127.0.0.1` игнор). distinct **/24** per юзер за окно = сигнал шеринга. **Калибровка 06-17: шеринга нет → enforcement НЕ вводим**, observe-only. Отчёт: `ip_usage_watcher.py --report`. Retention 48ч. Архитектура: `docs/plan-phase2-keystone-architecture.md`.
    - `0 12 * * 1` `scripts/vkturn_watch.sh` — **еженедельный твотчер релизов `cacggghp/vk-turn-proxy`** (whitelist-обход запаркован 06-23 из-за VK-капчи; новый релиз → TG-сигнал владельцу «ретест vk-turn»). State `/var/lib/vpn-health/vkturn_watch.state` (последний виденный тег; seed `v1.8.3`).
    После reboot проверять `crontab -l` — все на месте (текущий набор; eu1-share-audit снят 2026-06-15; vkturn_watch добавлен 06-23).
11. **Swap на eu1:** `/swapfile` 2 ГБ, `swappiness=10` (RAM всего 2 ГБ) — не удалять, страховка от OOM.
12. ~~**Swap на yc**~~ — **неактуально (yc снесён 06-28, снос YC).** Историч.: на yc стоял 1 ГБ swap (тесный free-tier RAM 960 МБ; Xray ел до 619 МБ → без swap OOM-freeze'ы).
13. **fail2ban на main** (с 2026-06-11, yc/yc2 снесены 06-28; Ubuntu 24.04, конфиг `/etc/fail2ban/jail.local`): `[sshd]` jail, `backend=systemd`, `maxretry=5`/`findtime=10m`/`bantime=1h`. **`ignoreip` ОБЯЗАТЕЛЬНО содержит Fornex `185.21.8.91`** — health_check / ip_usage_watcher / sync_xray_users / vless_summary_accounting ходят по SSH с Fornex; убрать из whitelist = fail2ban забанит мониторинг = каскадный сбой. `journalmatch` пиннится на `ssh.service` (Ubuntu-юнит, **не** `sshd.service` — иначе фильтр ловит 0 событий). **Fornex БЕЗ fail2ban** (jump-хост, динамический IP → риск самобана). health_check мониторит `fail2ban.service` на main. Снять бан: `fail2ban-client set sshd unbanip <ip>`. См. DONE_LIST 2026-06-11.
...
| Последняя сессия | `docs/sessions/SESSION_SUMMARY_2026-06-28.md` (🟢 блок 25.06 РЕШЁН — был `fp=chrome`+microsoft-SNI, не протокол/IP; REALITY воскрешён на ebay/deepl + firefox/qq; YC снесён полностью 06-28) |
