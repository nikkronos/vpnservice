...
10. **Cron на Fornex (root):**
    ...
    - `0 12 * * 1` `scripts/vkturn_watch.sh` — **еженедельный твотчер релизов форка `NikKuz99/vk-turn-proxy`** (апстрим `cacggghp` брошен → старый вотчер на нём был слеп; фикс VK-капчи пришёл форком v1.8.4, **ретест 06-29 ✅ капча СНЯТА**, трек запаркован по анти-масштабу, не технике). Новый релиз форка → TG-сигнал (актуален только под конкретного жёсткого-БС юзера / Phase B). State `/var/lib/vpn-health/vkturn_watch.state` (seed `v1.8.4`).
