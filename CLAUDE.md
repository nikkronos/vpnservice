...
10. **Cron на Fornex (root):**
    ...
    - `7 */6 * * *` `scripts/sync_xray_users.py --all --no-shared` — safety net: пересинхронизирует per-user VLESS UUIDs из БД в Xray config.json на main+yc+**yc2** (yc2 добавлен 2026-06-10 — клон yc, та же колонка `vless_uuid_yc`; без этого статичный клон дрейфовал бы при оплатах/истечениях). Защита от drift (ручные правки конфига / неудачные sync). 4 раза/день × ~5 сек downtime × 3 сервера. **Минута :07 (не :00) специально** — рестарт Xray не должен попадать на сетку health-check (`*/15` = :00/:15), иначе health-check ловит 5-сек окно рестарта → ложный `main:443 down` каждые 6 ч (исправлено 2026-06-05).
    ...
    - `*/15 * * * *` `scripts/health_check.py` — 29 проверок инфры (yc/yc2 +`:8443` socat sub-forward с 2026-06-10). **Fornex (13, локально):** systemd сервисы (vpn-bot/vpn-web/nginx/xray), docker (amnezia-awg2/mtproxy-faketls), AWG peer-count, peers.json/awg consistency, диск, swap, LE-cert, HTTPS endpoint, vless_config_consistency. **main (6, через SSH):** reachable + xray + wg-quick@wg0 + :443/tcp + :51820/udp + диск. **yc (4, через SSH):** reachable + xray + :443/tcp + диск. **yc2 (4, через SSH, РФ-резерв, добавлен 2026-06-10):** reachable + xray + :443/tcp + диск. **Все remote-проверки одного хоста идут одним SSH-batch'ем** (с маркерами `<<<BEGIN:N>>>`/`<<<END:N:rc>>>`) — раньше отдельные ssh-коннекты иногда отбивались провайдером main с `Connection closed by ... port 22` на 5-6-м соединении (false-FAIL'ы 2026-05-31). При смене статуса OK↔FAIL шлёт TG-алерт владельцу (прямой HTTP API, не через инстанс бота). State: `/var/lib/vpn-health/state.json`. Remote-ключи префиксованы `<host>:` (`yc:xray.service`). FAIL `<host>:reachable` — gate: остальные проверки этого хоста скипаются, чтобы один сетевой провал не давал каскад. **Кросс-серверная проверка `vless_config_consistency`** (с 2026-06-03, yc2 с 2026-06-10): count(active vless_uuid_X в users) vs count(clients в Xray config на main/yc/yc2) — FAIL если diff > 2 (yc2 сверяется с db_yc — клон yc).
    ...
    - **(ВРЕМЕННЫЙ, 2026-06-10)** `23 */6 * * *` `scripts/eu1_share_audit_sampler.sh` — liveness-аудит 9 инструментированных eu1 vless-tcp shared-UUID (`/var/log/eu1-share-audit.log`). Когда подтвердится «мертвы» (никто не использовался N дней) → удалить `sync_eu1_vless.py --no-shared --force` + снять этот cron + `instrument_eu1_shares.py`. См. DONE_LIST 2026-06-10.
    - `*/10 * * * *` `scripts/ip_usage_watcher.py` — **iplimit-НАБЛЮДЕНИЕ (Фаза 2 A-Stage1)**, ТОЛЬКО замер. Парсит access-log всех входов (eu1 локально + main/yc/yc2 SSH), копит (real client IP ↔ tid) в таблицу `ip_usage` (релей-строки `127.0.0.1` игнор). distinct-IP per юзер за окно = сигнал шеринга для калибровки порогов. Без enforcement. Отчёт: `ip_usage_watcher.py --report`. Retention 48ч. Архитектура: `docs/plan-phase2-keystone-architecture.md`.
...
| Последняя сессия | `docs/sessions/SESSION_SUMMARY_2026-06-11.md` |
