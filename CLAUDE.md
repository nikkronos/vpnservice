...
10. **Cron на Fornex (root):**
    ...
    - `7 */6 * * *` `scripts/sync_xray_users.py --all --no-shared` — safety net: пересинхронизирует per-user VLESS UUIDs из БД в Xray config.json на main+yc. Защита от drift (ручные правки конфига / неудачные sync). 4 раза/день × ~5 сек downtime = 20 сек/день суммарно. **Минута :07 (не :00) специально** — рестарт Xray не должен попадать на сетку health-check (`*/15` = :00/:15), иначе health-check ловит 5-сек окно рестарта → ложный `main:443 down` каждые 6 ч (исправлено 2026-06-05).
