...
10. **Cron на Fornex (root):**
    ...
    - `17 */6 * * *` `scripts/sync_eu1_vless.py --no-shared` — eu1-аналог sync_xray_users (eu1 ЛОКАЛЬНЫЙ, отдельный скрипт). Пересинхронизирует per-user VLESS UUIDs (колонка `vless_uuid_eu1`) в 3 inbound eu1 (vless-tcp/ws/xhttp) из БД, убирает истёкших (grace 12h). Добавлен 2026-06-10 после Этапа 4 закрытия фрод-зоны eu1 — без него eu1 per-user дрейфил (истёкшие висели, т.к. eu1 не было в рекуррентном sync; пример — `7882817110`). **Минута :17** — вне сетки health-check (:00/:15/:30/:45) и sync-xray (:07), чтобы рестарт eu1 xray не давал ложный FAIL.
    ...
    После reboot проверять `crontab -l` — все на месте (vpnservice-крон 10 шт + amnezia-save + certbot).
