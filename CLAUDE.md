...
- `📲 Получить VPN` → **сразу отдаёт `🔗 Подключить VPN`** (subscription `/sub/<token>`, телефон+ПК — главный путь, как в ЛК «всё на первом экране») + кнопка `🔌 Другие способы` → подменю `💻 AmneziaWG` (макс. скорость ПК/Wi-Fi, ⚠️ не для мобильного) / `📡 Мобильный` (под оператора main/eu1; yc убран 06-28)
- `🔄 Не работает` → подменю:
