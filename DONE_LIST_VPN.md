...
> Сжато 2026-06-13 (порядок в документах). Записей: 92 (включая 7 вех, ранее живших
> только в ROADMAP-пометках + сессиях — перенесены сюда при чистке ROADMAP).

## Оглавление

**2026-07**
- 2026-07-02 — Фикс биллинга: подтверждение оплаты (claim) падало на доставке подписки site-only юзеру (оплатил через ЛК не открыв бота → `_send_subscription`→`chat not found` рушил callback до перерисовки карточки/попапа; в БД оплата уже applied). Обёртка доставки в try + укреплён `_send_subscription` + `import sys` (VLESS auto-restore) + нотис владельцу. Выявлено реальным платежом; класс site-only (#2)

**2026-06**
- 2026-06-30 — vk-turn Phase B ВАЛИДИРОВАН (полный тоннель end-to-end, 1.7МБ через VK-релей→eu1→WG, серверно подтв.); жёсткий-БС тест Ани сорвался по топологии (БС только на её айфоне, ноут в норм. сети, тетеринг нельзя → iOS-only = iSH нереалистично); план: владелец self-test когда БС введут (Wi-Fi+VPN к Claude под БС жив), dormant-kit на eu1 (`up.sh/down.sh`) + рунбук на машине (`SESSION_SUMMARY_2026-06-30`)
- 2026-06-29 — vk-turn РЕТЕСТ: капча СНЯТА форком `NikKuz99` v1.8.4 (резид. РФ-IP → `VK Auth Success` + TURN-аллокация; блокер 06-23 `missing captcha_sid` снят) НО 9-мин cred-TTL + ручная капча = боутик → парк по анти-масштабу; вотчер→форк (seed v1.8.4); триаж уведомления (поймал не наш бот, а GitHub-PR-активность) (`SESSION_SUMMARY_2026-06-29`)
...
## 2026-07-02 — Фикс: approve claim падал на доставке подписки site-only юзеру

- **Симптом (владелец):** подтвердил оплату из ЛК-уведомления, но карточка осталась «Подписка неактивна» — без попапа «Подтверждено»; кнопки после повторного нажатия сняла ветка «уже approved». Выглядело как «не подтвердилось».
- **Корень:** юзер оплатил через webapp/ЛК, **не открывая бота** → `_send_subscription(tid)` в `callback_claim_decision` упал на `chat not found` (400). Вызов был не в try → исключение рушило callback ДО `edit_message_text` (перерисовка карточки) и `answer_callback_query`. В БД всё применилось (payment `succeeded`, подписка `active`, peer создан) — оплата фактически прошла, сломалась только обратная связь владельцу. Латентный баг: раньше платящие приходили через бота (чат существовал); первый чисто-site-only платёж (`elinanowikowa`) вскрыл.
- **Фикс (`bot/main.py`):** (1) `_restore_and_notify` + `_send_subscription` в approve обёрнуты в try/except → доставка юзеру больше не рушит подтверждение; (2) `_send_subscription` укреплён — при недоступном чате возвращает `False`, не бросает (польза всем payment-путям: Stars/donation/admin_credit); (3) `import sys as _sys` для async VLESS-restore (было `name 'sys' is not defined` → restore падал в warning); (4) в карточке approve — нотис «⚠️ юзер не открывал бота — заберёт подписку в ЛК».
- **Проверка:** локально `ast.parse` + серверный `py_compile`; деплой с бэкапом (`main.py.bak.<ts>`); smoke владельцем (заявка в боте → approve → карточка перерисовалась + попап).
- **Связь:** класс site-only / ghost-аккаунт — открытый пункт `ROADMAP_VPN` #2 (идентичность telegram vs email). Первый реальный site-only платёж как датапоинт.

## 2026-06-28 — 🟢 Инцидент VLESS-блока РЕШЁН (воскрешение)

Полная история — `SESSION_SUMMARY_2026-06-28.md`. Кратко:
- **Причина блока 25.06 (вскрыта через конкурент-интел VPN SPACE @vpn_spacev_bot):** дефолтные `fp=chrome` + `sni=www.microsoft.com` (заезжены пол-рынком → ТСПУ флагнул именно их), НЕ протокол REALITY, НЕ IP, НЕ порт. Вывод 06-26 «AWG = замена, REALITY мёртв» был НЕВЕРЕН.
...
Вскрыто через жалобу платящей (tid 1640320785, «не работает со вчера») → оказалось НЕ её кейс, а флот-обвал.
- **Факт:** весь VLESS-REALITY/TCP-443 (yc/yc2/eu1-direct/main) перестал нести трафик РФ-юзерам **25.06 ~11:00 UTC (14:00 МСК)**, мгновенный cliff (yc: 9147 req/ч в 10:00 → 28 в 11:00 → 0), ноль ≥26ч. Живёт только AmneziaWG (eu1 UDP, мобайл-зависимо). Подтверждено 2 РФ-вантажами (юзер + владелец: Европа/Европа-2/Германия/Россия в Happ мертвы, AWG на мобилке жив).
- **Диагноз (механизм):** блок на уровне **потребительских ISP/ТСПУ**, НЕ магистрали — серверы здоровы (REALITY-handshake отвечает из Германии И из РФ-ДЦ main; main достаёт yc/yc2/eu1 на 443; конфиги не менялись в момент обвала, правка только 00:07 рутинным sync). Бьёт 3 провайдера разом на TCP/443 + AmneziaWG-UDP на том же eu1-IP жив → **DPI VLESS-REALITY, не IP-блок**. РКН перешёл с IP-репутации (где Яндекс-IP выживал) на DPI-протокола → **Яндекс-релей больше не спасает**. Память `project_vpn_rkn_reality_dpi_block_0625`. Корреляция: Daniil (06-25 13:35 МСК) — вероятно передний край волны, не локальный кейс.
- **Слепое пятно health-check закрыто:** 26ч был зелёным (проверял порт+сервис, не реальный трафик). Добавлен `check_vless_traffic_flow` — возраст последнего `@kronos`-коннекта в access.log на yc+yc2; FAIL если ОБА воркхорса молчат ≥3ч при живом xray; SSH-сбой узла не учитывается (gate покроет, не плодит двойной алерт, ср. `project_vpn_main_ssh_double_alert`). Деплой через .new→syntax→бэкап→mv; проверено dry-run на живом обвале (`yc=1683м yc2=1640м → FAIL`, прочие 32 OK).
- **Компенсация простоя + инструмент:** всем активным (20: trial 5 / monthly 5 / referral 4 / 3dev_1m 3 / 3dev_0m 2 / 3dev_3m 1) выдано **+7 дней** — хирургично (только `expires_at` + сброс notif-флагов; status/plan/триал целы; бэкап БД перед применением, сверка по сэмплу +7д). Новый общий хелпер `db_bulk_extend_active(days)` + админ-кнопка **🎁 Дать всем дней** (бот → ⚙️ Администратор → число → подтверждение `grant_all_go:N`). Рассылка про временный переход на AmneziaWG отправлена владельцем.
- **Транспортный поиск (заменить мёртвый VLESS) — проведён:** REALITY-альт-порт 2053 мёртв (DPI по протоколу, не порт); Hysteria2 UDP/443 мёртв (QUIC-443 режут); Hysteria2 UDP/39590 ±Salamander-obfs — **Wi-Fi ок, мобила 0 коннектов** (QUIC ловят даже под obfs); **AmneziaWG работает на обеих сетях** → AWG строго доминирует, Hysteria снесён с eu1 начисто. Сервер+obfs валидированы локально (self-test). Клиент-грабли: self-signed ломает (**Happ 4.12.0 «XrayCore: allowInsecure removed»**), нужен реальный серт; Streisand тянет Hysteria2. Память `project_vpn_transport_after_block`; детали `SESSION_SUMMARY_2026-06-26`.
- **Вывод владельца:** AWG — отсрочка, не победа. Замена VLESS = AWG (eu1 flat-rate → сидестепит дорогой метрированный Яндекс-релей = корень убытка −2300₽/мес). Потолок: жёсткий whitelist-БС непробиваем ничем. **Открыт трек «обойти стену».**
- **Открыто (решение владельца):** приоритет трека обхода стены; сворот Яндекс-релея/yc2 (кейс усилился — REALITY на нём мёртв, трафик метрированный); AWG-позиционирование как основной путь (работает на мобиле вопреки «не для мобильного»); проверить Happ-4.12.0-allowInsecure на проде.
...
- **Жёсткий БС — итог обхода (внешние подтверждения):** опрос практиков (Артём: whitelisted-IP по договору = опознанный сервис, не серый; второй: CDN-фронт/STUN «для себя ок, масс быстро ловится») + наши тесты → **обход анти-масштабен; «обход» и «работает у всех» взаимоисключающи; killer-фича невозможна как масс-продукт.** Открыто: тест сервера знакомого Daniil (A БС снялся / B реально бьёт). Память `project_vpn_hard_whitelist_unbeaten`.
