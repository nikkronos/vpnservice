...
**Находки (прогон 06-10):**
- **main wg0: 4 пира, 2 живых ВНЕ таблицы** — `/5BeCINho` (tx 2 МБ, старый) + **`Mjvdy` (tx ~96 ГБ!)** — тяжёлый активный юзер, которого мы НЕ отслеживаем/не атрибутируем/не биллим. Доки говорили «1 legacy user» — по факту больше, один очень активный. **Владельцу: опознать, кто это** (друг/он сам/старый юзер) → решить track/leave.
- **6 legacy peer-строк** (eu2:2, rus1:3, rus2:1) — pubkey'ев нет НИ в одном runtime → **мёртвый cruft** (импорт 06-02, серверы сняты). Доступа не дают. Можно чистить (с go владельца) — чистая гигиена БД, не срочно.
