...
**2026-06**
- 2026-06-26 — 🔴 Флот VLESS-REALITY лёг (РКН-DPI на уровне ISP, 25.06 ~14:00 МСК): диагностика + закрыто слепое пятно health-check (`vless_traffic_flow`)
...
- **Открыто (НЕ в DONE, решение владельца):** ответ на блок — транспорт-диверсификация (AmneziaWG-центрично / REALITY на не-443 как дешёвый тест / Hysteria2-TUIC); текст юзерам о переходе на AWG; пересмотр роли YC (спец-преимущество Яндекс-IP нейтрализовано → ускоряет yc2-сворот: платим за нулевой трафик).
