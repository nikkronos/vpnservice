...
2. **eu1 (Европа, Fornex)**
   ...
   - **AmneziaWG:** развёрнут на eu1, из России работает (обфускация). Инструкция: docs/amneziawg-deploy-instruction.md, для пользователей: docs/client-instructions-amneziawg.md (iOS — импорт через «Поделиться»).
   - **Важно:** обычный WireGuard с eu1 из России не работает (блокировка на маршруте); для РФ — AmneziaWG.
