...
| MTProxy с Fake TLS (`mtproxy-faketls`, `nineseconds/mtg:2`) | main (Timeweb), порт 443 | ✅ Развёртывание: `docs/mtproxy-faketls-deploy.md`; алгоритм: `docs/telegram-unblock-algorithm.md` |
...
5. **Прокси Telegram:** на eu1 «голый» MTProto не использовать (блок по сигнатуре). Для Telegram без полного VPN — **MTProxy с Fake TLS на main** (`docs/mtproxy-faketls-deploy.md`). Команда `/proxy` в боте снята (2026-02-26); ссылка вручную / из `MTPROTO_PROXY_LINK` на сервере, не в репо
