...
- **Бот:** инструкции в боте охватывают ПК, iPhone/iPad и Android; AmneziaWG (Европа) — ПК, iOS (импорт через «Поделиться»), Android (импорт из файла/буфера). Чтобы бот **выдавал конфиги друзьям** (MVP): AmneziaWG на eu1 в Docker (amnezia-awg2) — развернуть скрипты `amneziawg-add-client-docker.sh` и `amneziawg-remove-client-docker.sh` на eu1, прописать пути в env на Timeweb; см. `docs/amneziawg-bot-automation-setup.md` (раздел «Вариант: AmneziaWG в Docker»).
...
## План «чистого сброса» eu1 (подключение есть, интернета нет)

Если на eu1 AmneziaWG: **подключение есть, интернета нет** (на телефоне/друзьях), а на ПК владельца работает — см. **`docs/eu1-clean-slate-plan.md`**. План: только Fornex (eu1); оставить MTProto (прокси для Telegram) и Shadowsocks; сбросить и заново установить AmneziaWG по инструкции; проверить на ПК и телефоне (второй iOS). Россия (main) не трогаем.

**Протоколы:** Telegram proxy = MTProto (Docker, 443) — оставляем. Shadowsocks (ss-redir) = для VPN+GPT/ChatGPT, не для Telegram — оставляем.
...
- **План чистого сброса eu1:** `docs/eu1-clean-slate-plan.md` — сброс только AmneziaWG на Fornex, оставить MTProto и Shadowsocks, установка с нуля.
- **Открытый вопрос (телефон: handshake есть, трафик не грузится) и варианты действий:** `docs/eu1-phone-config-open-question.md` — контекст, 6 вариантов для следующего агента (экспорт из приложения, сравнение формата, Xray и др.).
