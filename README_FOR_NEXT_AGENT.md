...
- **URL (сейчас, Timeweb):** [мониторинг](http://81.200.146.32:5001/) и [recovery](http://81.200.146.32:5001/recovery) — одна панель: главная **`/`** (статистика, сервисы, трафик) и **`/recovery`** (Telegram ID → ссылка MTProxy, VPN EU1/EU2).
...
- **Сервис:** `vpn-web.service` на Timeweb; **перенос на Fornex** (и `/`, и `/recovery`) — чеклист `docs/vpn-web-migration-fornex-plan.md` (на Fornex для трафика **main** нужны **`WG_SSH_*`** в `env_vars.txt`).
