...
- **Цель**: личный VPN/Proxy‑сервис для владельца и друзей, который:
  ...
  - в перспективе будет дополняться Telegram‑proxy (MTProto) и коммерческими тарифами.
...
- Оформить этот дизайн как спеки в `docs/specs/` (архитектура WireGuard+Shadowsocks, сценарии подключения, чеклисты).
- Добавить:
  - Telegram‑proxy (MTProto) на этом же сервере или на отдельном;
  - второй/резервный VPN‑сервер (например, отдельный EU‑сервер только под Shadowsocks/WireGuard);
  - автоматизацию выдачи конфигов через Telegram‑бота (генерация ключей, адресов и QR‑кодов).
