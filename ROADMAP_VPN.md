...
- [x] **Выгрузка пользователей (CRM-lite)** — 2026-05-15
  - [x] CSV-эндпоинт `/api/admin/users.csv` на веб-панели за `admin_key` — скачать актуальный CSV
  - [x] Синхронизация в Google Sheets через `bot/google_sheets.py` (`gspread` + service account)
  - [x] Кнопка «📊 Sync Google Sheets» в Telegram-боте (admin panel)
  - [x] Web-эндпоинт `POST /api/admin/sync-sheets` для программного вызова
  - [ ] Настроить на сервере: создать SA в Google Cloud, прописать `GOOGLE_SERVICE_ACCOUNT_JSON` и `GOOGLE_SHEETS_ID` в env_vars.txt
...
- [ ] **Key-based система вместо .conf** (по анализу конкурентов — 2026-05-08)
  ...
  - [x] Этап 2: интеграция в бота — /get_config выдаёт vless:// вместо .conf (2026-05-14)
    - [x] Абстракция сервера в БД (таблица `servers`, `db_pick_server()`) — 2026-05-15
    - [x] Поле `server_id` у пользователя — при создании выбирается наименее загруженный сервер
...
- [x] **Абстракция сервера в БД** — 2026-05-15
  - [x] Таблица `servers` в SQLite: `id`, `name`, `protocol` (vless/awg), `capacity`, `active`
  - [x] `db_pick_server(protocol)` — выбирает наименее загруженный активный сервер
  - [x] Убран хардкод `VLESS_SERVER_ID = "eu1"` из `vless_peers.py` → `get_vless_server_id()` через `db_pick_server()`
  - [x] `db_get_active_servers()`, `db_upsert_server()` — управление серверами через БД
  - [ ] IP-адреса и SSH-параметры пока остаются в env_vars.txt — перенести в таблицу `servers` при добавлении eu2
