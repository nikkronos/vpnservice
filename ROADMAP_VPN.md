...
- [ ] **Безопасность и rate limiting**
  - [ ] Rate limit на `/regen` — ограничить количество регенераций (например, не более 3 в сутки на пользователя), чтобы не спамили и не нагружали сервер
  - [ ] Аудит доступа: убедиться что приватные ключи Xray/WireGuard не утекают через бота или веб-панель
  - [ ] Проверить что `/recovery` сайт не отдаёт чужие конфиги (авторизация по Telegram ID)
