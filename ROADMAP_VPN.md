...
- [ ] **Веб-панель мониторинга** — приоритет: средний
  - [x] Создана базовая структура веб-панели (`web/`), Flask + API, шаблоны, статус серверов, статистика пользователей.
  - [ ] **Деплой веб-панели на сервере:** развернуть на Timeweb (или eu1), настроить systemd, получить **ссылку** на панель (сейчас ссылки нет).
  - [ ] **Доработать панель:** чтобы владелец мог всё мониторить (доступность нод, сервисы, подключения, при необходимости — графики/алерты). Зафиксировать URL панели в документации после деплоя.
...
- [ ] **Обход блокировок РКН (eu1 и устойчивость)** — приоритет: высокий
  ...
  - [x] **Доработать бота:** обновить инструкции в Telegram-боте — выдавать инструкцию по AmneziaWG (ПК + iOS через «Поделиться»), при возможности — выдавать конфиг/ссылку Amnezia — 2026-02-21.
    ...
    - [ ] Опционально: развернуть скрипт на eu1, задать AMNEZIAWG_EU1_ADD_CLIENT_SCRIPT — тогда бот будет выдавать AmneziaWG конфиги автоматически. Регенерация (/regen) для Европы пока вручную.
