...
- [ ] **Android: AmneziaWG "Unable to import tunnel: Invalid name"**
  - [ ] Причина: Linux ограничивает имя сетевого интерфейса до 15 символов. Имя берётся из имени файла `.conf` без расширения. Текущий паттерн `vpn_<telegram_id>_eu1_amneziawg` (~27 символов) — слишком длинный.
  - [ ] Решение: сократить имя файла, например `awg_eu1` или `vpn_eu1` (до 15 символов)
  - [ ] Обновить паттерн в боте (`bot/main.py`, функция `_do_get_config` и `_do_regen`)
  - [ ] Протестировать импорт на Android после исправления
  - [ ] Пока временное решение для таких пользователей: использовать Happ + VLESS через `/mobile_vpn`
