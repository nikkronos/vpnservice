...
- [ ] **Yota/MegaFon: YC-Reality не работает в whitelist-режиме**
  - [ ] Несколько подтверждений что Yota (сеть MegaFon) не пропускает трафик на YC IP `158.160.x.x`
  - [ ] Подтверждённые операторы: T2 ✅, МТС ✅. Yota ❌ (несколько случаев)
  - [ ] Варианты решения:
    - [ ] Попробовать другую подсеть YC (другая зона/регион)
    - [ ] Попробовать другой российский облачный провайдер в AS МегаФон
    - [ ] Изучить есть ли у МегаФона собственное облако, IP которого в whitelist
  - [ ] Проверить при следующем whitelist-событии (следующие праздники)
