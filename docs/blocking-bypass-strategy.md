...
- **С LTE не работает:**
  ...
  - Даже обычные запросы в браузере «висят» без завершения:
    - `http://185.21.8.91:5001/` (панель мониторинга, перенесена на Fornex)
...
- В проекте уже есть подтверждение аналогичного вывода для eu1: на LTE входящий TCP до Xray не фиксировался (см. `docs/archive/mobile-lte-eu1-xray-reality-attempt-2026-03.md`).
...
- **План резервов без нового VPS (и опционально второй VPS позже):** **`docs/specs/spec-08-multi-node-redundancy-ru2-eu2.md`** — сценарий B: main+eu1, при необходимости REALITY на main; сценарий A (RU2/EU2) — только при бюджете на отдельный IP/ASN.
...
### Дополнение 2026-05-05: режим «полного белого списка» на LTE (май 2026, СПб)

**Наблюдение:** в период предпраздничных ограничений (перед 9 мая 2026) мобильные операторы в Санкт-Петербурге перешли в режим **whitelist-only**: на LTE работают только ~2–3 одобренных ресурса. Все проверенные SIM-карты показывают одинаковое поведение.

**Что не работает в этом режиме:**
- AmneziaWG (UDP) — тайм-аут (было известно ранее).
- VLESS+REALITY прямое подключение к Fornex — IP не достигается.
- **Cloudflare CDN** (`sub.vpnnkrns.ru:443`) — даже IPs Cloudflare (104.x.x.x) не входят в whitelist; `https://sub.vpnnkrns.ru` не открывается в браузере.

**Ключевой вывод:** whitelist-режим блокирует **любые IP**, не входящие в список — включая крупнейшие CDN. Смена протокола, порта, транспорта (WebSocket/gRPC/XHTTP) не помогает: TCP-пакеты до любого незнакомого IP просто не проходят.

**Cloudflare CDN стек** (настроен в этой сессии) будет работать в **обычных условиях** (блок по ASN/IP конкретного VPS), но бессилен против тотального whitelist.

**Практические варианты при whitelist-режиме:**
1. Ждать снятия ограничений (временная мера).
2. Wi-Fi.
3. Исследовать конкретный whitelist — если в нём есть туннелируемый ресурс.

**Что НЕ работает и почему (проверено/подтверждено владельцем):**
- **Telegram** — блокируется так же жёстко как и другой интернет, не в whitelist
- **TimeWeb** — IP не входят в whitelist (именно поэтому перешли на Fornex)
- **Cloudflare CDN** — не в whitelist (проверено 2026-05-05)
- **Yandex Cloud VM** (`158.160.236.147`) — не в whitelist (проверено 2026-05-16)
- **e-SIM инструкции** — не делаем. Возможно только если сами станем агрегатором e-SIM (отдельное бизнес-направление, не подсказка пользователю)

**Вывод по whitelist:** список крайне узкий. Вероятно включает только госсервисы, Яндекс (домен), ВК, Сбер. Любой VPS/хостинг — под вопросом. Единственный перспективный путь — relay на IP который физически входит в whitelist (Yandex CDN или аналог).
...
- **`specs/spec-08-multi-node-redundancy-ru2-eu2.md`** — резервная нода (RU2/EU2), мульти-вход.
- **`third-party-vpn-boosters-vs-multi-entry.md`** — сторонние «ускорители» (GearUp и аналоги) vs наш подход.

---

*Документ создан 2026-02-21. Варианты 2.1–2.4 и совет по провайдерам/доменам добавлены 2026-02-21. Ссылки на spec-08 и third-party boosters — 2026-04-02.*
