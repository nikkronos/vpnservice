...
> ⚠️ **УСТАРЕЛО — классический WireGuard (история).** Документ описывает eu1 эпохи classic-WG: `wg0`, UDP **51820**, подсеть **10.1.0.0/24**, пиры по SSH с Timeweb, ручная правка `peers.json`. **Текущая реальность (2026):** eu1 = AmneziaWG в Docker-контейнере `amnezia-awg2` (интерфейс **awg0**, порт **39580/UDP**, подсеть **10.8.1.0/24**), управление через `docker exec`; бот + БД на **Fornex** (не Timeweb); **источник правды для peers — таблица `peers` в SQLite `vpn.db`**, НЕ `peers.json` (прямая правка JSON игнорируется — читается из таблицы). Команды `wg set wg0 …` и сниппет правки `peers.json` ниже — **НЕ применять.** Актуальная картина — в корневом `CLAUDE.md`.
