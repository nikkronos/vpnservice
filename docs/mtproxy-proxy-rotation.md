...
**Сводка для владельца (команды, recovery, env, деплой):** [telegram-mtproxy-operators-guide.md](telegram-mtproxy-operators-guide.md).
...
| Web recovery | [Страница восстановления](http://185.21.8.91:5001/recovery): после ввода Telegram ID ответ `POST /api/recovery/telegram-proxy` включает поле `mtproto_proxy_link` — **та же** ссылка, что по `/proxy`; при успешном перезапуске контейнера и при ошибке перезапуска (502) ссылка всё равно возвращается, чтобы пользователь мог открыть её вручную. |
...
## План внедрения на сервере (Fornex / где крутится бот)
...
- [vpn-web-migration-fornex-plan.md](vpn-web-migration-fornex-plan.md) — перенос страницы `/recovery` и `vpn-web` на Fornex (синхронизация ссылок с ботом).
