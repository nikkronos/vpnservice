...
| **ya.ru** (Яндекс, РФ; гео-консистентно с yc) | ya.ru:443 | ✅ TLS1.3+h2 + **живой T2-тест пройден** (подключился, выход через eu1) | 06-17 |
| **vk.com** (VK, РФ) | vk.com:443 | ✅ TLS1.3+h2 от yc (тот же класс, что вётнутый ya.ru) | 06-17 |
| **www.tbank.ru** (Т-Банк, РФ) | www.tbank.ru:443 | ✅ TLS1.3+h2 от yc (тот же класс) | 06-17 |
...
> валидные REALITY-dest, РФ-резидентные (переживают РКН по IP, гео-консистентны с yc).
> **ya.ru подтверждён живым T2-тестом** (временный inbound на yc2 :2096 + Happ на T2, VPN off →
> подключился, выход через eu1 185.21.8.91). vk.com / www.tbank.ru — тот же класс (РФ-гиганты,
> блок их SNI = массовый коллатерал → крайне маловероятен), считаем готовыми запасными.
> **Метод живого вёттинга** (для будущих кандидатов): поднять временный REALITY-inbound на
> свободном порту yc2 с нужным SNI + тестовый UUID → vless:// в Happ → проверить с T2 (VPN off) → снять.
