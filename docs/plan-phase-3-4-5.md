...
### 3i. Публичные страницы сайта (prerequisite для ЮKassa-модерации) — выявлено 2026-05-26

**Зачем**: ЮKassa при подключении проверяет сайт. Требует **публично доступные** (без авторизации):
- Описание услуги + цена.
- Оферта/пользовательское соглашение.
- Контакты + ФИО + ИНН самозанятого.

Сейчас на `https://supportkronos.online:8443/` стоит `@_require_admin_auth` (редирект на /login) → модератор откажет. **Это блокер для 3e (ЮKassa)** — без этого регистрация не пройдёт.

**Что сделать**:
- **Новый route `/`** (публичный, без auth) → `templates/landing.html`:
  - Заголовок + краткое описание услуги (нейтральное, без слова «VPN» — «доступ к интернет-сервису по подписке»).
  - УТП: «работает когда другие отваливаются» (под БС).
  - Цена: **200 ₽/мес**.
  - Кнопки: «Личный кабинет» (→ `/recovery`), «Оферта» (→ `/oferta`), «Канал» (→ `t.me/vforfriends`).
  - Подвал: ФИО владельца + ИНН + контактный email.
- **Новый route `/oferta`** (публичный) → `templates/oferta.html`:
  - Типовой договор оказания услуг для самозанятого (шаблон есть в интернете, адаптировать под нас).
  - Обязательно: реквизиты исполнителя (ФИО + ИНН), описание услуги, цена/порядок оплаты, права/обязанности, ответственность, контакты.
- **Новый route `/contacts`** или подвал лендинга — ФИО + ИНН + email + Telegram-канал.
- **Перенос админ-панели**: текущий `/` (admin index) → `/admin`. Логин остаётся `/login`.

**От владельца нужно** (до того как я смогу делать):
- ФИО полностью (как в паспорте).
- ИНН (как самозанятого, из «Мой налог»).
- Город регистрации (для оферты).
- Контактный email для публикации.
- Утверждение описания услуги (формулировка для модерации).

**Реализация**: Flask routes без декоратора `_require_admin_auth`, базовые HTML-шаблоны со стилями из текущего `style.css`. ~1-2 часа работы после получения данных от владельца.

### 3e. ЮKassa — основная оплата
**⚠️ Заблокирована Фазой 3i.** Сначала нужны публичные страницы.
