...
## Дополнение 2 (03.06) — диагностика массового T-7 reminder (НЕ баг)

Владелец заметил: «Подписка заканчивается через 7 дней» ушло многим разом, подумал что баг (даже сегодняшним). **Расследование → НЕ баг кода:**
- `db_users_due_for_expiry_notif(7)` корректна (T-7 только тем, у кого `date(expires_at)=сегодня+7`, grandfather исключён). Сегодняшним рег-юзерам НЕ уходило (они на 12-13 днях).
- **Корень:** при миграции на @vpnkronos 27.05 ~22:48 существующим юзерам выдали **14-дневный триал, НЕ grandfather** (как планировалось в Phase 4). ~30 юзеров (созданы 13–24 мая) имеют идентичный `expires_at=2026-06-10 22:48:20` → сегодня разом достигли T-7. Отправлено 19 (11 «retry» = заблокировали бота / тестовые tid). Владелец сам в когорте.
- **Решение владельца: ОСТАВИТЬ триал** (монетизация — кто не платит, отваливается 11.06 через `enforce_expired`). Reminder работает как задумано.
- Доки исправлены (CLAUDE.md биллинг-модель `b6d66ab` + память `project_vpn_decisions`): enforcement давно ВКЛючён, реальных grandfather-юзеров нет (только email-only placeholder 2099).
- **Тикающее:** 07.06 T-3, 10.06 T-0, 11.06 отрезание неоплативших. Владелец свой аккаунт продлит сам через `/admin`.

## Дополнение 3 (03.06) — улучшения админ-панели (commit `d9464b4`)

Две задачи из ROADMAP по запросу владельца (после разбора «зачем всё это» — он смотрел на панель):
- **Активность учитывает VLESS** (`/api/stats`): `active_24h/7d/30d` сводятся к `max(AWG handshake, VLESS last_seen, vless_requested_at)` per-юзер — **тот же сигнал, что бейджи** в `/api/traffic`. Было «3 за 24ч» (AWG-only) → стало **16** (реальная картина).
- **`/api/services` из health-check state**: читает `/var/lib/vpn-health/state.json`, маппит на **5 user-facing сервисов** (🛡️ AmneziaWG eu1 / 📲 VLESS 🇷🇺 Россия-main / 📲 VLESS 🇪🇺 Европа-yc / 🌐 VLESS-WS eu1 / 📎 MTProxy) + **staleness** (mtime>20мин→unknown — палит зависший health-check) + fallback на локальную проверку. Убран захардкоженный неверный IP `158.160.0.1`. Фронт (`index.html`+`main.js`) рендерит сервисы динамически.
- Задеплоено, проверено: 5 сервисов online, активность 16/20/20. Бэкапы `web/*.bak.20260603-153605`.

**Архитектурная заметка (для контекста):** у сервиса по факту **2 VLESS-REALITY сервера** — main (Timeweb/РФ, cloud.mail.ru, для Мегафон/Yota) + yc (Yandex Cloud, www.microsoft.com, для остальных). Оба в «Подписке» (метки «Россия»/«Европа») и в «Мобильном резерве». Третий VLESS — eu1 vless-ws (CDN, обычные блокировки), самый большой по трафику, legacy. AmneziaWG (eu1) — основной VPN.
