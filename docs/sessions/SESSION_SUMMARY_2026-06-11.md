...
> Большая сессия: **тарифы 199/249/449/599 НА ПРОДЕ**; **Фаза 2 B завершена
> (B4 устройства в ЛК + rename)**; **VLESS enforcement-лаг закрыт**; **yc2 ночной
> false-FAIL устранён** (ретрай SSH); **fail2ban на main/yc/yc2** (параллельный
> агент); гигиена доков. Порядок выбран владельцем: yc2-фикс → B4 → тарифы.

## TL;DR
- ✅ **Тарифы 199/249/449/599 НА ПРОДЕ** (`51292f3`, `feature/tariffs` → main). 2-шаговый выбор **устройства (3/5) → срок (1/3 мес)**; `bot/tariffs.py` — единый источник (цена на сервере); `device_limit` в DB (миграция, грандфазер 5 — на проде 58 юзеров=5); cap по тарифу в боте+ЛК. Бот-флоу + ЛК-picker + Stars-тиры (payload `stars_sub:tid:days:dl:ts`) + `/admin/credit` поле. Смоук владельца (бот+ЛК) ок. Детали: DONE_LIST 2026-06-11.
...
## ▶ Следующий заход
Тарифы / Фаза 2 B / VLESS-лаг / yc2 / fail2ban — **сделано в этой сессии**. Дальше:
1. **iplimit Stage 2** (~2026-06-17) — калибровка порогов по `ip_usage` → мягкий enforcement. Теперь у юзеров `device_limit` (3/5) — ловить шеринг сверх лимита.
2. **Авто-вердикт 9 eu1-shared** (~2026-06-16, сам прилетит в TG).
3. БС-валидация Яндекс-фронта (по событию). main-wg0 `/5BeCINho` (решение владельца).
4. *(опц.)* AWG endpoint-diversity детектор шеринга — зафлагован чипом `task_7262cac0`.

**Мелочи по тарифам:** бот-админ-кредит (`callback_admin_credit_user`) не трогает device_limit (веб-`/admin/credit` поле есть); триал = 5 устр. (грандфазер-дефолт).
