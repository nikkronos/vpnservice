...
- [x] (Опционально) Для eu1: добавить выбор «Обычный VPN» / «VPN+GPT»; при «VPN+GPT» — выделение IP из пула редиректа (10.1.0.8–254), создание peer, вызов на сервере скрипта добавления iptables, выдача конфига с пояснением «VPN+GPT».
- [x] На сервере eu1: пример скрипта `docs/scripts/add-ss-redirect.sh.example`; развёртывание и путь описаны в `docs/deployment.md`.
