...
> ⚠️ **РЕАЛИЗОВАНО (2026), спека историческая.** Концепт актуален (бот выдаёт AmneziaWG-конфиги для eu1), но детали устарели: eu1 теперь AmneziaWG в Docker `amnezia-awg2` (awg0, порт **39580**, подсеть **10.8.1.0/24**); источник правды peers — таблица SQLite. Актуальная картина — корневой `CLAUDE.md`.
